...
```{r}
df <- bind_rows(FirstDataframe,SecondDataFrame,ThirdDataFrame,FourthDataFrame,FifthDataFrame)
df
```
...
10 PRODUCTS - PRICE, RATINGS, and REVIEWS
```{r}
#PRODUCTS
session37 <- session("https://www.amazon.com.au/gp/bestsellers/beauty/5150070051/ref=zg_bs_nav_beauty_1")

tenProd <- session37 %>%
  html_nodes("div._cDEzb_p13n-sc-css-line-clamp-3_g3dy1") %>%
  html_text()

tenProdNames <- tenProd[10:19]
tenProdNames


#PRICE
session38 <- session("https://www.amazon.com.au/gp/bestsellers/beauty/5150070051/ref=zg_bs_nav_beauty_1")

tenPrice <-session38 %>%
  html_nodes("span.a-size-base") %>%
  html_text()
   
tenPrice1 <- tenPrice[10:19]
tenPrice1


#RATINGS
session39 <- session("https://www.amazon.com.au/gp/bestsellers/beauty/5150070051/ref=zg_bs_nav_beauty_1")

tenRatings <-session39 %>%
  html_nodes("span.a-size-small") %>%
  html_text()
   
tenRatings <- tenRatings[10:19]
tenRatings

#REVIEWS
session40 <- session("https://www.amazon.com.au/gp/bestsellers/beauty/5150070051/ref=zg_bs_nav_beauty_1")

tenRatings <-session40 %>%
  html_nodes("span.a-size-small") %>%
  html_text()
...
tenProdDf <- data.frame(
  Products = tenProdNames,
  Price = tenPrice1,
  Ratings = tenRatings,
  Reviews = tenRatings
)
...
write.csv(tenProdDf, file = "10 Products (Price, Ratings, and Reviews.csv")
```
